...
while total_length < target_length:
    new_block = [random.choice(alphabet) for _ in range(WORD_LEN)]
    reversed_block = reversed(new_block)
    blocks.append('F'+''.join(new_block))
    blocks.append('R'+''.join(reversed_block))
    total_length += 2*(len(new_block)+1)
data = '\n'.join(blocks)
